...
var formulabox_selector_rendered = formulabox_selector + ", figure > p > span[data-mathml]";
...
function bibliographic_reference_list() {
...
function captions() {
  $(figurebox_selector).each(function () {
    ...
    $(this).parents('figure').find('strong.cgen').remove()
  ...
  $(formulabox_selector).each(function () {
    ...
    var cur_number = $(this).findNumber(formulabox_selector_rendered);
    ...
    if ($.trim($(this).text()) == '') {
      ...
      if (referenced_element.length > 0) {
        ...
        referenced_element_formula = referenced_element.find(
          formulabox_selector_img + "," + formulabox_selector_span + "," + formulabox_selector_math + ",p > span[data-mathml]");
        ...
        if (
          ...
          var cur_count = referenced_element_formula.findNumber(formulabox_selector_rendered);
          if (cur_count != 0) {
            $(this).html("<span class=\"cgen\" data-rash-original-content=\"" + original_content +
...
function heading_dimensions() {
  $("h1").each(function () {
    var counter = 0;

    /** Must check if current heading is not title */
